...
```
code-time-tracker-vscode (本仓库)  ─┐
code-time-tracker (JetBrains)      ─┼─→  ctt-server  ←─  ctt-web
                                    │     (唯一后端)      (看板)
```

- **契约以 ctt-server 源码为准**，不以文档措辞为准（文档已发现多处与源码不一致）
- 统计语义由服务端裁决（plugin parity，服务端为 tie-breaker）；本仓库**不定义**统计口径
- 关联项目只读（AGENTS.md R3）；需要契约变更时写需求报告，不跨仓库改动

...
## P10. 语言归一化已在服务端实现，客户端不得代劳

**这是本领域最容易被「好心」做错的一条。** 服务端已实现跨端语言归一化
（`../ctt-server` 的 `language/` 包，v0.74.2 源码核实），客户端**唯一正确的动作是原样转发**。

| 送进去（原样） | 服务端归一为 |
|---|---|
| `typescript` / `TypeScript` / `TYPESCRIPT` | `TypeScript` |
| `ignore`（VS Code）与 `GitIgnore file`（JetBrains） | 都 → `Ignore List` |

大小写与首尾空白不敏感；**未识别的值原样保留**并标 `recognized=false`（供分类），
**不并入 `Other`** —— 所以送出未知值时不必担心它丢失。

**归一化在读取时发生**（`StatsCalculator` 查询期），列里始终是原始值 —— 历史数据因此
无需回填。理解这一点后，「本地先规范化再送」的诱惑就消失了：那只会与服务端结果打架。

客户端义务见 [`../sync-client/principles.md`](../sync-client/principles.md) P10。
